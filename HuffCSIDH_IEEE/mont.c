...
void biquad_both_Huff(fp *out,fp *outinv,const proj *P,const proj *Q,const proj *A)
{
  fp PxQx; fp_mul3(&PxQx,&P->x,&Q->x); //Xx
  fp PxQz; fp_mul3(&PxQz,&P->x,&Q->z); //Xz
  fp PzQx; fp_mul3(&PzQx,&P->z,&Q->x); //Zx
  fp PzQz; fp_mul3(&PzQz,&P->z,&Q->z); //Zz
  fp PPQQ; fp_mul3(&PPQQ,&PxQx,&PzQz); //XxZz
  fp PPQQc;
  fp_add2(&PPQQ,&PPQQ); //2XxZz
  fp_add3(&PPQQc,&PPQQ, &PPQQ); //PPQQc=4XxZz
   fp_add3(&PPQQ,&PPQQc, &PPQQc); //PPQQ=8XxZz
  fp_mul2(&PPQQ,&A->x); //2XxZzA


  fp s,t;

  fp_add3(&s,&PxQx,&PzQz); // s= Xx+Zz
  fp_add3(&t,&PzQx,&PxQz); // t = Zx+Xz
  fp_mul3(&out[1],&s,&t); // out1= (Xx+Zz)*(Zx+Xz)
  fp_add2(&out[1],&PPQQc);
  fp_mul2(&out[1],&A->z);
  fp_add2(&out[1],&PPQQ);
  fp_add2(&out[1],&out[1]);
  ...
  fp_sub3(&out[2],&PxQz,&PzQx); //out2=Xz-Zx
  fp_sq1(&out[2]); //(Xz-Zx)^2
  fp_mul2(&out[2],&A->z); //C(Xz-Zx)^2

  fp_sub3(&out[0],&PxQx,&PzQz); //Xx-Zz
  fp_sq1(&out[0]); //(Xx-Zz)^2
  fp_mul2(&out[0],&A->z);//C(Xx-Zz)^2
